...
echo "protID	bs_lnL	bsNull_lnL	m1a_lnL	bs_LRT	bs_p	relaxed_LRT	relaxed_p" > /uufs/chpc.utah.edu/common/HIPAA/u6035720/MURINAE/PARALOG_PROCESSING/SELEC_TESTS/pos_selec.bsTest.byExon.txt
ls -d /uufs/chpc.utah.edu/common/HIPAA/u6035720/MURINAE/PARALOG_PROCESSING/SELEC_TESTS/RTM_byExon-f16-seq20-site50_branchSite/* | while read dir;
do
	gene=$(echo "${dir}" | cut -d "/" -f 11 | cut -d "-" -f 1)
	echo "${gene}"
	lnL=$(grep "lnL" "${dir}/codeml.out" | cut -d ":" -f 4 | awk '{print $1}')
	echo "${lnL}"
	lnL_null=$(grep "lnL" /uufs/chpc.utah.edu/common/HIPAA/u6035720/MURINAE/PARALOG_PROCESSING/SELEC_TESTS/RTM_byExon-f16-seq20-site50_branchSiteNull/${gene}-mafft-cds.filter/codeml.out | cut -d ":" -f 4 | awk '{print $1}')
	echo "${lnL_null}"
	...
	echo "${p}"
	lnL_m1a=$(grep "lnL" /uufs/chpc.utah.edu/common/HIPAA/u6035720/MURINAE/PARALOG_PROCESSING/SELEC_TESTS/RTM_byExon-f16-seq20-site50_m1a/${gene}-mafft-cds.filter/codeml.out | cut -d ":" -f 4 | awk '{print $1}')
        echo "${lnL_null}"
	...
	echo "${p_relaxed}"
	printf "%s\t%f\t%f\t%f\t%s\t%f\t%s\t%f\n" "${gene}" "${lnL}" "${lnL_null}" "${lnL_m1a}" "${LRT}" "${p}" "${LRT_relaxed}" "${p_relaxed}" >> /uufs/chpc.utah.edu/common/HIPAA/u6035720/MURINAE/PARALOG_PROCESSING/SELEC_TESTS/pos_selec.bsTest.byExon.txt
done
