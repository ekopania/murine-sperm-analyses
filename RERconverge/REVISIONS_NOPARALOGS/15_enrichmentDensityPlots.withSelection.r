...
source("/uufs/chpc.utah.edu/common/HIPAA/u6035720/MURINAE/RERconverge/MariaBarcodePlotFinal.colorBySelec.R")
#relax<-read.table("/uufs/chpc.utah.edu/common/HIPAA/u6035720/MURINAE/DNDS_BY_SPERM_STAGE/PAML/pos_selec.pValCor.sigOnly.posOnly.m1aVbsnull.txt", header=TRUE)
relax<-read.csv("relax_output.GOspermatogenesisProteins_OUfg_fullReproSet.BHcorrection.sigOnly.relaxedOnly.csv", header=TRUE)
intense<-read.csv("relax_output.GOspermatogenesisProteins_OUfg_fullReproSet.BHcorrection.sigOnly.intensifiedOnly.csv", header=TRUE)
bustedph<-read.table("GOspermatogenesisProteins_OUfg_fullReproSet.selecAssocTrait.txt", header=TRUE, sep="\t")
...
intense_genes<-unique(sort(unlist(sapply(intense$protID, function(x) all_genes$gene_id[which(all_genes$prot_id == x)]))))
bustedph_genes<-unique(sort(unlist(sapply(bustedph$protID, function(x) all_genes$gene_id[which(all_genes$prot_id == x)]))))
#overlap<-Reduce(intersect, list(relax_genes_all, intense_genes_all, bustedph_genes_all))
#relax_genes<-relax_genes_all[which(!(relax_genes_all %in% overlap))]
#intense_genes<-intense_genes_all[which(!(intense_genes_all %in% overlap))]
#bustedph_genes<-bustedph_genes_all[which(!(bustedph_genes_all %in% overlap))]
...
for(i in rownames(rhoMat)){
	if(i %in% bustedph_genes){
		selec_status<-c(selec_status, 1)
	} else if(i %in% intense_genes){
		selec_status<-c(selec_status, 2)
	} else if(i %in% relax_genes){
...
print(paste("Number of genes with selection associated w/ small testes:", length(which(rhoMat_selec_spermOnly[,'selec_status']==1))))
print(paste("Number of genes under intensified selection:", length(which(rhoMat_selec_spermOnly[,'selec_status']==2))))
print(paste("Number of genes under relaxed selection:", length(which(rhoMat_selec_spermOnly[,'selec_status']==3))))
...
myCol<-c("#CCCCCC", "#984447", "#3E4768", "#028090", "#000000")
#none = #CCCCCC, busted-ph = #984447, intensified = #3E4768, relaxed = #028090, worm = #000000
...
GO_spermatogenesis_bustedph<-intersect(rownames(rhoMat_selec)[which(rhoMat_selec[,'selec_status']==1)], GO_spermatogenesis)
print(length(GO_spermatogenesis_bustedph))
makeMultiGSEAplotSelection(rhoMat_selec, GO_spermatogenesis_bustedph, span=.2, title="GO category: spermatogenesis - selection associated with small testes", cols=c("#984447","#984447"))
GO_spermatogenesis_intense<-intersect(rownames(rhoMat_selec)[which(rhoMat_selec[,'selec_status']==2)], GO_spermatogenesis)
print(length(GO_spermatogenesis_intense))
makeMultiGSEAplotSelection(rhoMat_selec, GO_spermatogenesis_intense, span=.2, title="GO category: spermatogenesis - intensified selection", cols=c("#3E4768","#3E4768"))
GO_spermatogenesis_relaxed<-intersect(rownames(rhoMat_selec)[which(rhoMat_selec[,'selec_status']==3)], GO_spermatogenesis)
