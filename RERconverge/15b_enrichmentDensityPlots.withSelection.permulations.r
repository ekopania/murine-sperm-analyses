...
for(i in RERrun){
	#load(paste0("RERconverge_output.logRTM", i, ".permulationsSSM.customSSM.customSisters.rds"))
	load(paste0("RERconverge_output.logRTM", i, ".permulationsSSM.customSSM.fromMasterTree.rds"))
	res<-permpvalSSM
	print(head(res))
	...
	if(i %in% c("_binary.OUmodel.RTMspeciesOnly", "_binary.q10.RTMspeciesOnly", "_binary.q20.RTMspeciesOnly", "_binary.q50low.RTMspeciesOnly")){
		rhoMat<-cbind(rhoMat, res_hasGeneID$permstats)
	} else{
		rhoMat<-cbind(rhoMat, -(res_hasGeneID$permstats))
	}
	if(is.vector(rhoMat)){
...
pdf("enrichment_density.OUmodel.span0_2.withSelection.permulationsSSM.fromMasterTree.pdf", height=7, width=10, onefile=TRUE)
